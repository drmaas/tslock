...
function makeDatastore(overrides: Record<string, any> = {}) {
  const runTransaction = vi.fn();
  const key = vi.fn((parts: string[]) => ({ kind: parts[0], name: parts[1] }));
  return { runTransaction, key, ...overrides } as any;
}

function makeTxn(overrides: Record<string, any> = {}) {
  const get = vi.fn();
  const upsert = vi.fn();
  return { get, upsert, ...overrides } as any;
}
...
describe('DatastoreStorageAccessor', () => {
  ...
  describe('field encoding', () => {
    it('writes ISO strings by default', async () => {
      ...
      const data = txn.upsert.mock.calls[0][0].data;
