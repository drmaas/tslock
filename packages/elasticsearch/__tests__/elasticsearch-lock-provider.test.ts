...
function client(overrides: Record<string, unknown> = {}): Client {
  return { update: vi.fn().mockResolvedValue({ result: 'updated' }), ...overrides } as unknown as Client;
}
...
describe('ElasticsearchLockProvider', () => {
  afterEach(() => {
  ...
  it('lock() returns lock when update returns updated', async () => {
    const c = client();
    const provider = new ElasticsearchLockProvider(c, { index: 'shedlock-test' });
    const lock = await provider.lock(config());
    expect(lock).toBeDefined();
  });

  it('lock() returns lock when update returns created (upsert)', async () => {
    const c = client({ update: vi.fn().mockResolvedValue({ result: 'created' }) });
  ...
  it('lock() returns undefined when update returns noop', async () => {
    const c = client({ update: vi.fn().mockResolvedValue({ result: 'noop' }) });
    const provider = new ElasticsearchLockProvider(c);
    const lock = await provider.lock(config());
  ...
  it('lock() passes refresh: wait_for', async () => {
    const fn = vi.fn().mockResolvedValue({ result: 'updated' });
